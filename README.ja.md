...
現在は Windows x64 をサポートしています。**Linux（NVIDIA DGX を含む）および macOS 対応を予定**しています。Linux/macOS は `curl | tar`、Windows は `NSIS`/`MSI` を使用します。Windows リリースインストーラーはリリース公開用証明書が設定されている場合に Authenticode で署名されます。公開された署名と SHA-256 チェックサムを確認してください。
