    ...
    <div className="app-page-scroll discover-panel relative flex h-full min-h-0 flex-col p-4">
      <div className="mb-4 flex min-w-0 flex-wrap items-end justify-between gap-3">
        ...
        <div className="rounded-lg border border-slate-800 bg-slate-900 px-3.5 py-2 text-right text-[11px] text-slate-500">
          ...
          <div className="mt-0.5 max-w-[18rem] truncate text-slate-300" title={store.cfg?.models_dir ? normalizeDisplayPath(store.cfg.models_dir) : pt(locale, "loading")}>{store.cfg?.models_dir ? normalizeDisplayPath(store.cfg.models_dir) : pt(locale, "loading")}</div>
        </div>
      </div>
      ...
      <form onSubmit={(event) => { event.preventDefault(); void search(); }} className="mb-4 flex min-w-0 gap-2.5" aria-busy={searching || loadingFiles || downloading !== null}>
        ...
        <input id="discover-search" value={query} onChange={(event) => setQuery(event.target.value)} placeholder={xt(locale, "searchPlaceholder")} className="app-input flex-1 h-10 px-4 text-sm" />
        <button type="submit" disabled={searching} className="discover-search-button app-button app-button--primary app-button--lg shrink-0">{searching ? pt(locale, "scanning") : pt(locale, "searchModels")}</button>
      </form>
      ...
      <div className="app-panel-feedback-layer" aria-live="polite">
        ...
        <div className="discover-progress-card rounded-xl border border-indigo-800/80 bg-indigo-950/30 p-4" role="status">
          <div className="flex items-center justify-between gap-3 text-xs">
            <span className="min-w-0 truncate text-indigo-200">{ut(locale, "downloadingFile", { file: normalizeDisplayPath(progress.file_path) })}</span>
            <span className="shrink-0 text-indigo-300">{progressPercent}%</span>
          </div>
          <div className="mt-2.5 h-2 overflow-hidden rounded-full bg-slate-800" role="progressbar" aria-label={ut(locale, "downloadProgress")} aria-valuemin={0} aria-valuemax={100} aria-valuenow={progress.total > 0 ? progressPercent : undefined}><div className="h-full rounded-full bg-indigo-500 transition-[width]" style={{ width: `${progressPercent}%` }} /></div>
          <div className="mt-2.5 flex items-center justify-between gap-3 text-[11px] text-slate-500"><span>{ut(locale, "bytesOfTotal", { received: formatBytes(progress.received), total: formatBytes(progress.total) })}</span><button type="button" onClick={() => void cancel()} className="app-button app-button--danger app-button--sm">{pt(locale, "cancel")}</button></div>
        </div>
        )}
      </div>
      ...
      <div className="grid min-h-0 flex-1 gap-4 lg:grid-cols-[minmax(18rem,0.85fr)_minmax(24rem,1.15fr)]">
        <section className="min-h-0 overflow-auto rounded-xl border border-slate-800 bg-slate-900/40" aria-label={xt(locale, "searchResults")}>
          <div className="sticky top-0 z-10 border-b border-slate-800 bg-slate-950/90 px-4 py-3 text-xs font-semibold uppercase tracking-wide text-slate-500">{xt(locale, "searchResults")} {results.length ? `(${results.length})` : ""}</div>
          ...
          <div role="list">
            {results.map((model) => (
              <div key={model.id} role="listitem"><button type="button" onClick={() => void inspect(model)} className={`block w-full border-b border-slate-800 px-4 py-3 text-left last:border-0 hover:bg-slate-800/70 focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-inset focus-visible:ring-indigo-400 ${selected?.id === model.id ? "bg-indigo-950/50" : ""}`}>
            ...
            {!loadingFiles && files && files.length > 0 && <div role="list">
              {files.map((file) => {
                ...
                return <div key={file.path} role="listitem" className="flex min-w-0 flex-wrap items-center justify-between gap-3 border-b border-slate-800 px-4 py-3 last:border-0">
                  <div className="min-w-0 flex-1"><div className="truncate text-sm font-medium text-slate-200" title={displayFilePath}>{displayFilePath}</div><div className="mt-1 flex flex-wrap gap-2 text-[11px] text-slate-500"><span>{formatBytes(file.size_bytes)}</span><span>{file.is_mmproj ? ut(locale, "visionProjector") : quantLabel(file.path)}</span>{file.oid && <span title={file.oid}>{ut(locale, "checksumMetadata")}</span>}</div></div>
                  <button type="button" onClick={() => void download(file)} disabled={!!downloading || !["stopped", "failed", "crashed"].includes(store.status.state)} title={!["stopped", "failed", "crashed"].includes(store.status.state) ? ut(locale, "stopBeforeDownload") : undefined} className="app-button app-button--primary app-button--sm shrink-0">{activeDownload ? `${xt(locale, "downloading")}…` : file.is_mmproj ? xt(locale, "downloadProjector") : xt(locale, "download")}</button>
                </div>;
              })}
            </div>}
          </>}
