  ...
  return (
    <div className="app-page-scroll developer-panel relative flex h-full min-h-0 flex-col overflow-auto p-4" data-developer-section={section}>
      ...
      {section === "api" && <div className="developer-summary-grid mb-4 grid gap-3 sm:grid-cols-3" role="group" aria-label={t("panel.ariaDeveloperSummary")}>
        <div className="flex flex-col justify-center rounded-lg border border-slate-800 bg-slate-900/50 p-3.5"><div className="text-[10px] uppercase tracking-wide text-slate-500">{t("ui.localApi")}</div><div className="mt-1 text-sm font-medium text-slate-200">{serverReady ? t("panel.ready") : t("panel.offline")}</div><div className="mt-1 truncate font-mono text-[10px] text-slate-500" title={baseUrl || t("ui.startServerForUrl")}>{baseUrl || t("ui.startServerForUrl")}</div></div>
        <div className="flex flex-col justify-center rounded-lg border border-slate-800 bg-slate-900/50 p-3.5"><div className="text-[10px] uppercase tracking-wide text-slate-500">{t("ui.loadedModels")}</div><div className="mt-1 text-sm font-medium text-slate-200">{models.length || "—"}</div><div className="mt-1 text-[10px] text-slate-500">{t("ui.fromModelsEndpoint")}</div></div>
        <div className="flex flex-col justify-center rounded-lg border border-slate-800 bg-slate-900/50 p-3.5"><div className="text-[10px] uppercase tracking-wide text-slate-500">{t("ui.gateway")}</div><div className="mt-1 text-sm font-medium text-slate-200">{gateway.running ? t("ui.running") : t("ui.stopped")}</div><div className="mt-1 text-[10px] text-slate-500">{t("ui.localhostOnly")}</div></div>
      </div>}
      ...
      {section === "api" && <section className="developer-section developer-section--connection grid items-start gap-4 lg:grid-cols-[minmax(0,1.3fr)_minmax(17rem,0.7fr)]">
        <div className="rounded-xl border border-slate-800 bg-slate-900/50 p-4">
          ...
          <div className="mt-3.5 grid gap-3 sm:grid-cols-2"><div className="flex flex-col justify-between rounded-lg border border-slate-800 bg-slate-950/70 p-3"><div className="text-[11px] uppercase tracking-wide text-slate-600">{t("ui.baseUrl")}</div><div className="mt-1 break-all font-mono text-xs text-cyan-300">{baseUrl || "http://127.0.0.1:8080/v1"}</div><button type="button" onClick={() => void copy("base-url", baseUrl)} disabled={!baseUrl} className="app-button app-button--secondary app-button--sm mt-2.5 self-start">{copied === "base-url" ? t("panel.copied") : t("ui.copyUrl")}</button></div><div className="flex flex-col justify-between rounded-lg border border-slate-800 bg-slate-950/70 p-3"><div className="text-[11px] uppercase tracking-wide text-slate-600">{t("ui.authorization")}</div><div className="mt-1 font-mono text-xs text-slate-300">Bearer &lt;LOCAL_API_KEY&gt;</div><div className="mt-2 text-[11px] text-slate-600">{t("ui.keyInMemory")}</div></div></div>
          <div className="mt-3.5 flex flex-wrap gap-2 text-xs text-slate-500"><span className="rounded app-bg-muted px-2.5 py-1">state: {store.status.state}</span>{store.status.pid && <span className="rounded app-bg-muted px-2.5 py-1">PID: {store.status.pid}</span>}{store.status.model && <span className="max-w-full truncate rounded app-bg-muted px-2.5 py-1" title={normalizeDisplayPath(store.status.model)}>effective: {normalizeDisplayPath(store.status.model).split(/[\\/]/).pop()}</span>}<span className="rounded app-bg-muted px-2.5 py-1">{t("ui.authLocalBearer")}</span></div>
        </div>
        <div className="rounded-xl border border-slate-800 bg-slate-900/50 p-4"><h3 className="app-section-title">{t("ui.loadedModels")}</h3><p className="app-section-hint">{t("ui.loadedModelsHint")}</p>{!serverReady && <div className="mt-4 text-sm text-slate-600">{t("ui.serverOffline")}</div>}{serverReady && models.length === 0 && !loadingModels && <div className="mt-4 text-sm text-slate-600">{t("ui.noModelResponse")}</div>}{models.map((model) => { const displayModelId = normalizeDisplayPath(model.id); return <div key={model.id} className="mt-3 rounded-lg border border-slate-800 bg-slate-950/70 p-2.5"><div className="truncate font-mono text-xs text-slate-200" title={displayModelId}>{displayModelId}</div><div className="mt-1 text-[11px] text-slate-600">{model.owned_by || "llama.cpp"}</div></div>; })}</div>
      </section>}

      {section === "api" && <section className="developer-section developer-section--endpoints mt-4 rounded-xl border border-slate-800 bg-slate-900/50 p-4"><div className="flex items-start justify-between gap-3"><div><h3 className="app-section-title">{t("ui.endpointsTitle")}</h3><p className="app-section-hint">{t("ui.endpointsHint")}</p></div><span className="rounded app-bg-muted px-2.5 py-1 text-[11px] text-slate-500">/v1</span></div><div className="mt-3.5 grid gap-3 md:grid-cols-2">{ENDPOINTS.map((endpoint) => { const id = `${endpoint.method}-${endpoint.path}`; const snippet = buildCurlSnippet(baseUrl || "http://127.0.0.1:8080/v1", endpoint.path); return <div key={id} className="rounded-lg border border-slate-800 bg-slate-950/60 p-3.5"><div className="flex items-center gap-2"><span className="rounded bg-cyan-950 px-2 py-0.5 font-mono text-[10px] text-cyan-300">{endpoint.method}</span><code className="font-mono text-xs text-slate-200">/v1{endpoint.path}</code></div><p className="mt-2 text-xs leading-relaxed text-slate-500">{t(`ui.${endpoint.description}`)}</p><button type="button" onClick={() => void copy(id, snippet)} className="app-button app-button--secondary app-button--sm mt-2.5">{copied === id ? t("ui.copiedCurl") : t("ui.copyCurl")}</button></div>; })}</div></section>}
      ...
      {section === "api" && <section className="developer-section developer-section--compatibility mt-4 grid gap-4 lg:grid-cols-2">
        <div className="rounded-xl border border-slate-800 bg-slate-900/50 p-4">
          ...
          <div className="mt-2.5 flex flex-wrap gap-1.5 text-[11px] text-slate-500"><span className="rounded app-bg-muted px-2.5 py-1">stateful chat</span><span className="rounded app-bg-muted px-2.5 py-1">load/unload</span><span className="rounded app-bg-muted px-2.5 py-1">download status</span></div>
        ...
        <div className="rounded-xl border border-slate-800 bg-slate-900/50 p-4">
          ...
          <div className="mt-2.5 flex flex-wrap gap-1.5 text-[11px] text-slate-500"><span className="rounded app-bg-muted px-2.5 py-1">thinking blocks</span><span className="rounded app-bg-muted px-2.5 py-1">tool use</span><span className="rounded app-bg-muted px-2.5 py-1">SSE translation</span></div>
      ...
      {section === "gateways" && <section className="developer-section developer-section--gateway mt-4 rounded-xl border border-slate-800 bg-slate-900/50 p-4"><div className="developer-section-header flex min-w-0 flex-wrap items-start justify-between gap-3"><div className="developer-section-copy min-w-0"><h3 className="app-section-title">{t("ui.gatewayTitle")}</h3><p className="app-section-hint max-w-2xl">{t("ui.gatewayHint")}</p></div><button type="button" onClick={() => void toggleGateway()} disabled={!serverReady} className="developer-section-action app-button app-button--primary app-button--sm">{gateway.running ? t("ui.stopGateway") : t("ui.startGateway")}</button></div>{gateway.running && <div className="developer-section-status mt-3 flex min-w-0 flex-wrap items-center gap-2 text-xs text-slate-400"><span className="rounded bg-emerald-950 px-2 py-1 text-emerald-300">{t("ui.running")}</span><code className="developer-code-block min-w-0 break-all text-fuchsia-300">{gateway.url}</code></div>}<div className="mt-2 text-[11px] text-slate-600">{t("ui.gatewayBindHint")}</div></section>}

      {section === "gateways" && <section className="developer-section developer-section--responses mt-4 rounded-xl border border-slate-800 bg-slate-900/50 p-4"><div className="developer-section-header flex min-w-0 flex-wrap items-start justify-between gap-3"><div className="developer-section-copy min-w-0"><h3 className="app-section-title">{t("ui.responsesTitle")}</h3><p className="app-section-hint max-w-2xl">{t("ui.responsesHint")}</p></div><span className="developer-section-status rounded bg-cyan-950 px-2.5 py-1 text-[11px] text-cyan-300">{gateway.running ? t("ui.responsesAvailable") : t("ui.responsesStartGateway")}</span></div><div className="developer-responses-grid mt-3.5 grid min-w-0 gap-3 sm:grid-cols-2"><code className="developer-code-block min-w-0 break-all rounded-lg bg-slate-950 p-3 font-mono text-[11px] text-cyan-300">{gateway.url ? gateway.url.replace(/\/v1\/messages\/?$/, "") + "/v1/responses" : "http://127.0.0.1:8081/v1/responses"}</code><code className="developer-code-block min-w-0 break-all rounded-lg bg-slate-950 p-3 font-mono text-[11px] text-cyan-300">GET/DELETE /v1/responses/&lt;id&gt;</code></div><button type="button" onClick={() => void copy("responses", `curl ${gateway.url ? gateway.url.replace(/\/v1\/messages\/?$/, "") + "/v1/responses" : "http://127.0.0.1:8081/v1/responses"} \\\n  -H "Authorization: Bearer ***" \\\n  -H "Content-Type: application/json" \\\n  -d '{"model":"<MODEL_ID>","input":"Hello","previous_response_id":null,"stream":false}'`)} disabled={!gateway.running} className="app-button app-button--secondary app-button--sm mt-3">{copied === "responses" ? t("ui.copiedCurl") : t("ui.copyResponsesCurl")}</button></section>}

      {section === "api" && <section className="developer-section developer-section--snippets mt-4 grid gap-4 lg:grid-cols-2"><div className="rounded-xl border border-slate-800 bg-slate-900/50 p-4"><div className="flex items-center justify-between gap-3"><h3 className="app-section-title">Python</h3><button type="button" onClick={() => void copy("python", pythonSnippet)} className="app-button app-button--secondary app-button--sm">{copied === "python" ? t("panel.copied") : t("ui.copy")}</button></div><pre className="mt-3 max-h-56 overflow-auto whitespace-pre-wrap break-words rounded-lg bg-slate-950 p-3 font-mono text-[11px] leading-relaxed text-slate-400">{pythonSnippet}</pre></div><div className="rounded-xl border border-slate-800 bg-slate-900/50 p-4"><div className="flex items-center justify-between gap-3"><h3 className="app-section-title">JavaScript</h3><button type="button" onClick={() => void copy("javascript", jsSnippet)} className="app-button app-button--secondary app-button--sm">{copied === "javascript" ? t("panel.copied") : t("ui.copy")}</button></div><pre className="mt-3 max-h-56 overflow-auto whitespace-pre-wrap break-words rounded-lg bg-slate-950 p-3 font-mono text-[11px] leading-relaxed text-slate-400">{jsSnippet}</pre></div></section>}

      {section === "diagnostics" && <section className="developer-section developer-section--diagnostics mt-4 rounded-xl border border-slate-800 bg-slate-900/50 p-4"><h3 className="app-section-title">{t("ui.diagnosticsTitle")}</h3><pre className="mt-3 max-h-80 overflow-auto whitespace-pre-wrap break-words rounded-lg p-3 font-mono text-xs leading-relaxed">{normalizeDisplayText(store.status.log_tail || store.status.error || t("ui.noDiagnostics"))}</pre></section>}

      {section === "api" && serverReady && <div className="developer-api-footer mt-4 text-[11px] text-slate-600">{t("ui.exampleEndpoint")}: <span className="font-mono text-slate-500">{endpointUrl(baseUrl, "/models")}</span>. {t("ui.keyNeverRendered")}</div>}
    </div>
  );
}
