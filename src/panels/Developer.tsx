...
export default function DeveloperPanel({ store, section = "api" }: { store: AppStore; section?: "api" | "gateways" | "diagnostics" }) {
  ...
  const sectionHeading = section === "gateways"
    ...
    : section === "diagnostics"
      ...
      : { title: t("section.api"), description: ut(locale, "apiDescription") };
  ...
  return (
    <div className="developer-panel flex h-full min-h-0 flex-col overflow-auto p-3 sm:p-4" data-developer-section={section}>
      <div className="mb-4 flex min-w-0 flex-wrap items-end justify-between gap-3">
        <div className="min-w-0"><div className="text-xs font-semibold uppercase tracking-[0.18em] text-cyan-300">{t("section.developer")}</div><h2 className="mt-1 text-xl font-semibold tracking-tight text-slate-100">{sectionHeading.title}</h2><p className="mt-1 max-w-2xl text-sm leading-relaxed text-slate-500">{sectionHeading.description}</p></div>
        <StatusBadge label={serverReady ? pt(locale, "apiReady") : pt(locale, "startServer")} tone={serverReady ? "success" : "warning"} />
      </div>
      <div className="mb-3 grid gap-2 sm:grid-cols-3" role="group" aria-label={pt(locale, "ariaDeveloperSummary")}>
        <div className="rounded-lg border border-slate-800 bg-slate-900/50 p-3"><div className="text-[10px] uppercase tracking-wide text-slate-500">{ut(locale, "localApi")}</div><div className="mt-1 text-sm text-slate-200">{serverReady ? pt(locale, "ready") : pt(locale, "offline")}</div><div className="mt-1 break-all font-mono text-[10px] text-slate-500">{baseUrl || ut(locale, "startServerForUrl")}</div></div>
        <div className="rounded-lg border border-slate-800 bg-slate-900/50 p-3"><div className="text-[10px] uppercase tracking-wide text-slate-500">{ut(locale, "loadedModels")}</div><div className="mt-1 text-sm text-slate-200">{models.length || "—"}</div><div className="mt-1 text-[10px] text-slate-500">{ut(locale, "fromModelsEndpoint")}</div></div>
        <div className="rounded-lg border border-slate-800 bg-slate-900/50 p-3"><div className="text-[10px] uppercase tracking-wide text-slate-500">{ut(locale, "gateway")}</div><div className="mt-1 text-sm text-slate-200">{gateway.running ? ut(locale, "running") : ut(locale, "stopped")}</div><div className="mt-1 text-[10px] text-slate-500">{ut(locale, "localhostOnly")}</div></div>
      </div>

      {error && <FeedbackBanner tone="error" title={t("error.wrong")} onDismiss={() => setError(null)}>{error}</FeedbackBanner>}

      {!serverReady && !error && <FeedbackBanner tone="info" title={pt(locale, "apiUnavailable")} action={{ label: pt(locale, "startServer"), onClick: () => void store.start().catch(() => undefined) }}>{pt(locale, "startLocalServer")}</FeedbackBanner>}
      ...
      <section className="developer-section developer-section--connection grid gap-3 lg:grid-cols-[minmax(0,1.3fr)_minmax(17rem,0.7fr)]">
        <div className="rounded-xl border border-slate-800 bg-slate-900/50 p-4">
          <div className="flex flex-wrap items-start justify-between gap-3"><div><h3 className="text-sm font-semibold text-slate-200">{ut(locale, "connection")}</h3><p className="mt-1 text-xs text-slate-500">{ut(locale, "connectionHint")}</p></div><button type="button" onClick={() => void refreshModels()} disabled={!serverReady || loadingModels} className="rounded-lg border border-slate-700 bg-slate-800 px-3 py-1.5 text-xs text-slate-300 hover:bg-slate-700 disabled:opacity-40 focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-cyan-400">{loadingModels ? ut(locale, "checking") : ut(locale, "checkModels")}</button></div>
          <div className="mt-3 grid gap-2 sm:grid-cols-2"><div className="rounded-lg border border-slate-800 bg-slate-950/70 p-3"><div className="text-[11px] uppercase tracking-wide text-slate-600">{ut(locale, "baseUrl")}</div><div className="mt-1 break-all font-mono text-xs text-cyan-300">{baseUrl || "http://127.0.0.1:8080/v1"}</div><button type="button" onClick={() => void copy("base-url", baseUrl)} disabled={!baseUrl} className="mt-2 rounded bg-slate-800 px-2 py-1 text-[11px] text-slate-400 hover:text-white disabled:opacity-40">{copied === "base-url" ? pt(locale, "copied") : ut(locale, "copyUrl")}</button></div><div className="rounded-lg border border-slate-800 bg-slate-950/70 p-3"><div className="text-[11px] uppercase tracking-wide text-slate-600">{ut(locale, "authorization")}</div><div className="mt-1 font-mono text-xs text-slate-300">Bearer &lt;LOCAL_API_KEY&gt;</div><div className="mt-2 text-[11px] text-slate-600">{ut(locale, "keyInMemory")}</div></div></div>
          <div className="mt-3 flex flex-wrap gap-2 text-xs text-slate-500"><span className="rounded bg-slate-800 px-2 py-1">state: {store.status.state}</span>{store.status.pid && <span className="rounded bg-slate-800 px-2 py-1">PID: {store.status.pid}</span>}{store.status.model && <span className="max-w-full truncate rounded bg-slate-800 px-2 py-1" title={store.status.model}>effective: {store.status.model.split(/[\\/]/).pop()}</span>}<span className="rounded bg-slate-800 px-2 py-1">{ut(locale, "authLocalBearer")}</span></div>
        </div>
        <div className="rounded-xl border border-slate-800 bg-slate-900/50 p-4"><h3 className="text-sm font-semibold text-slate-200">{ut(locale, "loadedModels")}</h3><p className="mt-1 text-xs text-slate-500">{ut(locale, "loadedModelsHint")}</p>{!serverReady && <div className="mt-4 text-sm text-slate-600">{ut(locale, "serverOffline")}</div>}{serverReady && models.length === 0 && !loadingModels && <div className="mt-4 text-sm text-slate-600">{ut(locale, "noModelResponse")}</div>}{models.map((model) => <div key={model.id} className="mt-3 rounded-lg border border-slate-800 bg-slate-950/70 p-2.5"><div className="truncate font-mono text-xs text-slate-200" title={model.id}>{model.id}</div><div className="mt-1 text-[11px] text-slate-600">{model.owned_by || "llama.cpp"}</div></div>)}</div>
      </section>

      <section className="developer-section developer-section--endpoints mt-3 rounded-xl border border-slate-800 bg-slate-900/50 p-4"><div className="flex items-start justify-between gap-3"><div><h3 className="text-sm font-semibold text-slate-200">{ut(locale, "endpointsTitle")}</h3><p className="mt-1 text-xs text-slate-500">{ut(locale, "endpointsHint")}</p></div><span className="rounded bg-slate-800 px-2 py-1 text-[11px] text-slate-500">/v1</span></div><div className="mt-3 grid gap-2 md:grid-cols-2">{ENDPOINTS.map((endpoint) => { const id = `${endpoint.method}-${endpoint.path}`; const snippet = buildCurlSnippet(baseUrl || "http://127.0.0.1:8080/v1", endpoint.path); return <div key={id} className="rounded-lg border border-slate-800 bg-slate-950/60 p-3"><div className="flex items-center gap-2"><span className="rounded bg-cyan-950 px-1.5 py-0.5 font-mono text-[10px] text-cyan-300">{endpoint.method}</span><code className="font-mono text-xs text-slate-200">/v1{endpoint.path}</code></div><p className="mt-2 text-xs leading-relaxed text-slate-500">{ut(locale, endpoint.description)}</p><button type="button" onClick={() => void copy(id, snippet)} className="mt-2 rounded bg-slate-800 px-2 py-1 text-[11px] text-slate-400 hover:text-white focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-cyan-400">{copied === id ? ut(locale, "copiedCurl") : ut(locale, "copyCurl")}</button></div>; })}</div></section>

      <section className="developer-section developer-section--compatibility mt-3 grid gap-3 lg:grid-cols-2">
        <div className="rounded-xl border border-slate-800 bg-slate-900/50 p-4">
          <h3 className="text-sm font-semibold text-slate-200">{ut(locale, "lmStudioTitle")}</h3>
          <p className="mt-1 text-xs leading-relaxed text-slate-500">{ut(locale, "lmStudioHint")}</p>
          ...
          <button type="button" onClick={() => void copy("native-chat", `curl ${rootUrl}/api/v1/chat \\\n  -H "Authorization: Bearer <LOCAL_API_KEY>" \\\n  -H "Content-Type: application/json" \\\n  -d '{"model":"<MODEL_ID>","input":"Hello","stream":true}'`)} className="mt-3 rounded bg-slate-800 px-2 py-1 text-[11px] text-slate-400 hover:text-white">{copied === "native-chat" ? ut(locale, "copiedCurl") : ut(locale, "copyNativeCurl")}</button>
        </div>
        <div className="rounded-xl border border-slate-800 bg-slate-900/50 p-4">
          <h3 className="text-sm font-semibold text-slate-200">{ut(locale, "anthropicTitle")}</h3>
          <p className="mt-1 text-xs leading-relaxed text-slate-500">{ut(locale, "anthropicHint")}</p>
          ...
          <button type="button" onClick={() => void copy("anthropic-messages", `curl ${rootUrl}/v1/messages \\\n  -H "x-api-key: <LOCAL_API_KEY>" \\\n  -H "anthropic-version: 2023-06-01" \\\n  -H "Content-Type: application/json"`)} className="mt-3 rounded bg-slate-800 px-2 py-1 text-[11px] text-slate-400 hover:text-white">{copied === "anthropic-messages" ? ut(locale, "copiedCurl") : ut(locale, "copyMessagesCurl")}</button>
        </div>
      </section>

      <section className="developer-section developer-section--gateway mt-3 rounded-xl border border-slate-800 bg-slate-900/50 p-4"><div className="flex flex-wrap items-start justify-between gap-3"><div><h3 className="text-sm font-semibold text-slate-200">{ut(locale, "gatewayTitle")}</h3><p className="mt-1 max-w-2xl text-xs leading-relaxed text-slate-500">{ut(locale, "gatewayHint")}</p></div><button type="button" onClick={() => void toggleGateway()} disabled={!serverReady} className="rounded-lg border border-fuchsia-800 bg-fuchsia-950/50 px-3 py-1.5 text-xs text-fuchsia-200 hover:bg-fuchsia-900 disabled:opacity-40">{gateway.running ? ut(locale, "stopGateway") : ut(locale, "startGateway")}</button></div>{gateway.running && <div className="mt-3 flex flex-wrap items-center gap-2 text-xs text-slate-400"><span className="rounded bg-emerald-950 px-2 py-1 text-emerald-300">{ut(locale, "running")}</span><code className="break-all text-fuchsia-300">{gateway.url}</code></div>}<div className="mt-2 text-[11px] text-slate-600">{ut(locale, "gatewayBindHint")}</div></section>

      <section className="developer-section developer-section--responses mt-3 rounded-xl border border-slate-800 bg-slate-900/50 p-4"><div className="flex flex-wrap items-start justify-between gap-3"><div><h3 className="text-sm font-semibold text-slate-200">{ut(locale, "responsesTitle")}</h3><p className="mt-1 max-w-2xl text-xs leading-relaxed text-slate-500">{ut(locale, "responsesHint")}</p></div><span className="rounded bg-cyan-950 px-2 py-1 text-[11px] text-cyan-300">{gateway.running ? ut(locale, "responsesAvailable") : ut(locale, "responsesStartGateway")}</span></div><div className="mt-3 grid gap-2 sm:grid-cols-2"><code className="break-all rounded-lg bg-slate-950 p-3 font-mono text-[11px] text-cyan-300">{gateway.url ? gateway.url.replace(/\/v1\/messages\/?$/, "") + "/v1/responses" : "http://127.0.0.1:8081/v1/responses"}</code><code className="break-all rounded-lg bg-slate-950 p-3 font-mono text-[11px] text-cyan-300">GET/DELETE /v1/responses/&lt;id&gt;</code></div><button type="button" onClick={() => void copy("responses", `curl ${gateway.url ? gateway.url.replace(/\/v1\/messages\/?$/, "") + "/v1/responses" : "http://127.0.0.1:8081/v1/responses"} \\\n  -H "Authorization: Bearer ***" \\\n  -H "Content-Type: application/json" \\\n  -d '{"model":"<MODEL_ID>","input":"Hello","previous_response_id":null,"stream":false}'`)} disabled={!gateway.running} className="mt-3 rounded bg-slate-800 px-2 py-1 text-[11px] text-slate-400 hover:text-white disabled:opacity-40">{copied === "responses" ? ut(locale, "copiedCurl") : ut(locale, "copyResponsesCurl")}</button></section>

      <section className="developer-section developer-section--snippets mt-3 grid gap-3 lg:grid-cols-2"><div className="rounded-xl border border-slate-800 bg-slate-900/50 p-4"><div className="flex items-center justify-between gap-3"><h3 className="text-sm font-semibold text-slate-200">Python</h3><button type="button" onClick={() => void copy("python", pythonSnippet)} className="rounded bg-slate-800 px-2 py-1 text-[11px] text-slate-400 hover:text-white">{copied === "python" ? pt(locale, "copied") : ut(locale, "copy")}</button></div><pre className="mt-3 max-h-56 overflow-auto whitespace-pre-wrap break-words rounded-lg bg-slate-950 p-3 font-mono text-[11px] leading-relaxed text-slate-400">{pythonSnippet}</pre></div><div className="rounded-xl border border-slate-800 bg-slate-900/50 p-4"><div className="flex items-center justify-between gap-3"><h3 className="text-sm font-semibold text-slate-200">JavaScript</h3><button type="button" onClick={() => void copy("javascript", jsSnippet)} className="rounded bg-slate-800 px-2 py-1 text-[11px] text-slate-400 hover:text-white">{copied === "javascript" ? pt(locale, "copied") : ut(locale, "copy")}</button></div><pre className="mt-3 max-h-56 overflow-auto whitespace-pre-wrap break-words rounded-lg bg-slate-950 p-3 font-mono text-[11px] leading-relaxed text-slate-400">{jsSnippet}</pre></div></section>

      <section className="developer-section developer-section--diagnostics mt-3 rounded-xl border border-slate-800 bg-slate-900/50 p-4"><h3 className="text-sm font-semibold text-slate-200">{ut(locale, "diagnosticsTitle")}</h3><pre className="mt-3 max-h-80 overflow-auto whitespace-pre-wrap break-words rounded-lg p-3 font-mono text-xs leading-relaxed">{store.status.log_tail || store.status.error || ut(locale, "noDiagnostics")}</pre></section>

      {serverReady && <div className="mt-3 text-[11px] text-slate-600">{ut(locale, "exampleEndpoint")}: <span className="font-mono text-slate-500">{endpointUrl(baseUrl, "/models")}</span>. {ut(locale, "keyNeverRendered")}</div>}
    </div>
  );
}
